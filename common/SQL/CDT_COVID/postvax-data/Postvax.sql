...
SELECT DATE,
    ...
    BOOST_CASE_RATE,
    BOOST_HOSP_RATE,
    BOOST_DEATH_DOD_RATE,
    REPORT_DATE
FROM COVID.DEVELOPMENT.CDPH_POSTVAX_CASE_HOSP_DEATH_BOOSTED 
WHERE (REPORT_DATE=(select max(REPORT_DATE) from COVID.DEVELOPMENT.CDPH_POSTVAX_CASE_HOSP_DEATH_BOOSTED)) 
      AND (BOOST_CASE_RATE IS NOT NULL)
      AND AREA='California'
ORDER BY DATE
;