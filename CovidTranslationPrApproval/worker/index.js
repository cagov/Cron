...
const doTranslationPrUpdate = async masterbranch => {
    ...
    for (const prlist of Prs) {
        ...
        if (pr.mergeable) {
            ...
            //Do not allow non-printable characters.  New Lines,tabs and Arabic number shifts are ok + 8294+8297+65279+8203(zero width space).
            ...
            if (pass && fileaccessok && !badCharFiles.length) {
                ...
                badCharFiles.forEach(x=>{
                    console.error(`Bad file char (${JSON.stringify(x.badChar[0])}) in ${x.filename}`);
                });
            }
        }
    }
